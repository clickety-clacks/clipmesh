...
| LocalAPI HTTP framing compatibility | `crates/clipmesh-hub-edge/tests/fixtures/localapi-http-framing-v1.json` | Sanitized regression shape from bounded compatibility evidence. It pins the accepted LocalAPI Host value and chunked status and WhoIs responses while keeping only reserved example payload values. |
