...
| LocalAPI HTTP framing compatibility | `crates/clipmesh-hub-edge/tests/fixtures/localapi-http-framing-v1.json` | Explicit synthetic parser regression, not captured LocalAPI bytes. It pins the accepted LocalAPI Host value and synthetic chunked status and WhoIs responses while keeping only reserved example payload values. |
